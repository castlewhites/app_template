        ...
        rx.hstack(
            rx.flex(
               rx.box(
                    *[
                        rx.vstack(
                            rx.text(str(num), font_size="80px", color="red") if info == "Usuarios por validar" else rx.text(str(num), font_size="80px", color="#535353"),
                            rx.text(info, size="2", color="#666", margin_right="6px", text_align="center"),
                            spacing="20px", 
                            align_items="center"
                        )
                        for info, num in data["user_counts"].items()
                    ],
                    display="flex",
                    width="48%",
                    padding="10px",
